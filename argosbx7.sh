...
agsbxurl="https://raw.githubusercontent.com/hostwebgood/ygkkk-argosbx/refs/heads/main/argosbx7.sh"
showmode(){
echo "Argosbx脚本一键SSH命令生器在线网址：https://yonggekkk.github.io/argosbx/"
echo "主脚本：bash <(curl -Ls https://raw.githubusercontent.com/hostwebgood/ygkkk-argosbx/refs/heads/main/argosbx7.sh) 或 bash <(wget -qO- https://raw.githubusercontent.com/hostwebgood/ygkkk-argosbx/refs/heads/main/argosbx7.sh)"
...
ss_link3="- {name: \"${sxname} Shadowsocks\", type: ss, server: $server_ip1, port: $port_ss, cipher: 2022-blake3-chacha20-poly1305, password: $sskey, udp: true, smux: { enabled: true, protocol: 'h2mux', padding: true, max-connections: '8', min-streams: '16', statistic: true, only-tcp: false } }"
ss_link5="
  {
    \"type\": \"shadowsocks\",
    \"tag\": \"${sxname} Shadowsocks\",
...
echo "💣【 TUIC 】节点信息如下："
port_tu=$(cat "$HOME/agsbx/port_tu")
tuic5_link="tuic://$uuid:$uuid@$server_ip:$port_tu/?congestion_control=bbr&udp_relay_mode=native&alpn=h3&sni=player.live-video.net&allow_insecure=1&pinSHA256=${FP_SHA256}#${sxname} TUIC"
...
cleandel
rm -rf "$HOME/agsbx"/{sb.json,xr.json,sbargoym.log,sbargotoken.log,argo.log,argoport.log,cdnym,name,private.key,cert.pem}
command -v openssl >/dev/null 2>&1 && openssl ecparam -genkey -name prime256v1 -out "$HOME/agsbx/private.key" >/dev/null 2>&1
command -v openssl >/dev/null 2>&1 && openssl req -new -x509 -days 363 -key "$HOME/agsbx/private.key" -out "$HOME/agsbx/cert.pem" -subj "/CN=player.live-video.net" >/dev/null 2>&1
FP_SHA256=$(openssl x509 -fingerprint -noout -sha256 -in $HOME/agsbx/cert.pem 2>/dev/null | awk -F= '{print $NF}')
FP_BASE64=$(openssl x509 -in $HOME/agsbx/cert.pem -pubkey -noout | openssl pkey -pubin -outform der | openssl dgst -sha256 -binary | openssl enc -base64 2>/dev/null)
...
echo "纯IPv6网络环境，可考虑自行添加NAT64或启用WARP。"
fi
