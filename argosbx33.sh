...
hy2_link="hy2://$uuid@$server_ip:$port_hy2/?&mport=$mport&insecure=1&sni=player.live-video.net&hop_interval=17&hpkp=${FP_SHA256}#${sxname} Hysteria2"
hy2_link1="hysteria2://$uuid@$server_ip:$port_hy2/?&mport=$mport&insecure=1&sni=player.live-video.net&hop_interval=17&pinSHA256=${FP_SHA256}#${sxname} Hysteria2"
hy2_link3="{name: \"${sxname} Hysteria2\", type: hysteria2, server: $server_ip, port: $port_hy2, ports: $cmhy2pt, hop-interval: 17, password: $uuid, sni: player.live-video.net, skip-cert-verify: false, fingerprint: ${FP_SHA256}}"
hy2_link5="{ \"type\": \"hysteria2\", \"tag\": \"${sxname} Hysteria2\", \"server\": \"$server_ip\", \"server_port\": \"$port_hy2\", \"server_ports\":[\"$sbhy2pt\"], \"hop_interval\": \"17s\", \"hop_interval_max\": \"30s\", \"password\": \"$uuid\", \"tls\": { \"enabled\": true, \"server_name\": \"player.live-video.net\", \"certificate_public_key_sha256\": [\"${FP_BASE64}\"], \"alpn\": [ \"h3\" ] } },"
